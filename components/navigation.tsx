...
import { cn } from "@/lib/utils" // Optional, but recommended for cleaner class merging
...
export function Navigation() {
  ...
  // 2. Get the current path and determine if we are on the homepage
    ...
    <svg width="70" height="70" viewBox="0 0 300 300" fill="none" xmlns="http://www.w3.org/2000/svg">
      ...
      <path d="M139.2 98.2264C143.256 92.7244 147.312 86.2984 151.728 80.3164V98.7904C147.672 104.496 143.778 110.394 138.876 115.338C134.178 120.078 128.874 125.178 122.61 127.746C115.662 130.638 109.8 130.638 106.14 130.8C103.65 130.92 99.0715 130.356 100.434 128.91C102.642 126.582 116.016 122.844 120.834 119.148C129.078 112.848 133.176 106.38 139.2 98.2264Z" fill="currentColor"/>
        ...
        <nav className="hidden md:flex items-center space-x-8">
          <Link href="/" className={cn("text-sm font-light transition-colors", navTextColor, hoverTextColor)}>
            Home
          </Link>
          <Link href="/products" className={cn("text-sm font-light transition-colors", navTextColor, hoverTextColor)}>
            Products
          </Link>
        </nav>
        ...
        <div className="flex items-center space-x-2">
          <CartDrawer>
            <Button variant="ghost" size="icon" className={cn("relative", navTextColor, hoverTextColor)}>
              {/* This SVG and all others now use `currentColor` for stroke */}
          ...
          <Sheet open={isOpen} onOpenChange={setIsOpen}>
            ...
            {/* Remember to also style your SheetContent based on isHomePage if needed! */}
            <SheetContent>
